...
terminalStates (_,term,_) = term

transitions :: Automaton -> [Transition]
transitions (_,_,trans) = trans

isTerminal :: State -> Automaton -> Bool
isTerminal s auto = elem s (terminalStates auto) 

transitionsFrom :: State -> Automaton -> [Transition]
transitionsFrom s auto = [t | t@(x,_,_)<- (transitions auto) , x == s]

labels :: [Transition] -> [Label] 
labels a = nub $ labels' a
  where
    labels' [] = [] 
    labels' ((_,_,C l):xs) = (C l) : labels' xs
    labels' (_:xs) = labels' xs

accepts :: Automaton -> String -> Bool
accepts auto str = accepts' (startState auto) str 
    where 
      accepts' :: State -> String -> Bool
      accepts' s str
        | isTerminal s auto && null str = True
        | otherwise = or $ map (try str) transitions
          where
            transitions = transitionsFrom s auto 

      try :: String -> Transition -> Bool
      try str (_,target,Eps) = accepts' target str 
      try str@(s:xs) (_,target,C a)
          | null str = False
          | s == a = accepts' target (xs)
          | otherwise = False
...
make Null m n k = ([(m,n,Eps)], k + 1) 
make (Term c) m n k = ([(m,n,C c)], k + 1) 
make (Seq r1 r2) m n k = (trans'++trans''++[(k,k + 1,Eps)],k'')
  where 
    (trans',k')   = make r1 m k (k+1)
    (trans'',k'') = make r2 (k+1) n k'
make (Alt r1 r2) m n k = (trans'++trans''++[(m,k,Eps),(k+1,n,Eps),(m,k+2,Eps),(k+3,n,Eps)],k'')
  where 
    (trans',k')   = make r1 k (k+1) (k+3)
    (trans'',k'') = make r2 (k+2) (k+3) k'
make (Rep r1) m n k  = (trans' ++ [(m,k,Eps),(k+1,k,Eps),(k+1,n,Eps),(m,n,Eps)],k')
  where
    (trans',k')   = make r1 k (k+1) (k+2)

make re m n k   = make (simplify re) m n k
--------------------------------------------------------
-- Part IV
