...
getAttribute s (Element _ attributes _)
    = fromMaybe "" (lookup s attributes)
getAttribute _ _  = ""

getChildren :: String -> XML -> [XML]
getChildren name xml = getChildren' xml
  where
    getName (Element name _ _) = name
    getName _ = ""

    getChildren' (Element _ _ xml_lst) 
      = filter (\x -> name == getName x) xml_lst
    getChildren' _ = []
      

getChild :: String -> XML -> XML
getChild name xml 
  | null $ getChildren name xml = (Text "")
  | otherwise = head $ getChildren name xml

addChild :: XML -> XML -> XML
-- Pre: the second argument is an Element
addChild xml1 (Element name attr xml_lst)
  = (Element name attr (xml_lst ++ [xml1]))

getValue :: XML -> XML
getValue x = Text (getValue' x)
  where 
    getValue' (Text a) = a
    getValue' (Element _ _ xml_lst) 
      =  concatMap getValue' xml_lst
-------------------------------------------------------------------------
-- Part II
...
addText str (x:xs) = (addChild (Text str) x : xs) 
...
parseAttributes str = (lst_of_attrbts, tail waste)
  where
    (attrbts_str,waste) = break ((==) '>' ) str
    lst_of_attrbts = parseAttributes' attrbts_str []
    parseAttributes' [] attr = attr
    parseAttributes' txt@(x:xs) attr
      = parseAttributes' (skipSpace (tail xs_attr)) ((name,val):attr)
        where 
          (name, xl) = parseName (skipSpace txt)
          (_, xt)  = break ('\"' == ) xl
          (val,xs_attr) = break ('\"' == ) (tail xt)
